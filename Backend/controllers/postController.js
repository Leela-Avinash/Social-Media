...
const commentOnPost = async (req, res) => {
    try {
        ...
        const comment = await Comment.create({postid, commentedby, text, userprofilepic, username});
...
const getFeed = async (req, res) => {
    try {
        ...
        if(!user) {
            return res.status(400).json({message: "Please login to view posts"});
        }
...
export {createPost, getPost, deletePost, likePost, commentOnPost, getFeed};